...
function approveTask (task) {
  const id = task.id
  ShRepo.runPlainRequest('tasks/approve/'+id ,'Are you sure you want to approve this task?')
      .then(res=>{
        if(res.isConfirmed){
          ShRepo.showToast('Task approved successfully', 'success')
          reload.value++
        }
      })
      .catch(err=>console.log(err))
